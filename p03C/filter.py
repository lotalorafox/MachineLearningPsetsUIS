...
def plot_kalman_position_velocity(phist, mhist, xhist, Phist, vhist=None, ylim=None):
    ...
    plot_moves_variance(real=vhist, measures=None, xhist=velocities,  Phist=[i[1,1] for i in Phist], ylim = (vmin-np.abs(vmin)*0.2,
                                                                                                             vmax+np.abs(vmax)*0.2))
    plt.title("velocity")
...
def plot_all(wsize, lmarks, robot_positions, particles, errs_mean, errs_std, parts_std):
